...
if __name__ == "__main__":
    ...
    # grid search xyz配列
    # read image
    image = cv2.imread('../image/r_grid.jpg')
    height = image.shape[0]
    width = image.shape[1]
    ...
    for h in range(height):
        for w in range(width):
            ...
            if depth_type == 1:
                ...
                grid[size_i][2] = height / 2 - h -100 # z
            else:
    ...
    for grid_i in range(len(grid)): # grid配列に入れられた要素をすべて検証
        ...
        h = int(grid_i / width)
        w = int(grid_i % width)
        if image[h][w][2] > 50 and image[h][w][0] < 50:
            # 検索対象座標
            x = grid[grid_i][0]
            y = grid[grid_i][1]
            z = grid[grid_i][2]
    ...
    for i in range(len(cylinder)):
        if red[i] == 1:
            result[int(cylinder[i][1])][int(cylinder[i][0])][0] = 0
            result[int(cylinder[i][1])][int(cylinder[i][0])][1] = 0
    ...
    while True:
        ...
        cv2.imshow("detected.jpg", result)
